...
class DelayMenu extends MusicScene {
	...
	override function create() {
		...
		for (i in 0...objects.length) {
			props.push(Sprite('stages/stage/${objects[i].name}', objects[i].pos[0], objects[i].pos[1]).setScale(.7));
			add(props[i]);
		}

		props[3].flipX = true;

		add(bf = Sparrow('ui/options/bf', 429, 282).addPrefix('idle', 'idle', 24, false).playAnim('idle').finishAnim());
	...
	function onBack() {
		conductor.paused = Controls.block = true;
